...
/**
 * @file advanced_example_main.c
 * @brief Advanced CLI-API example with argument-based commands, system, WiFi, and NVS
 *
 * This example demonstrates:
 * - cli_register_command() with typed arguments (INT, STRING, FLAG)
 * - cli_register_commands() for batch registration
 * - Integration with cmd_system, cmd_wifi, and cmd_nvs components
 * - Input validation and hardware interaction (GPIO)
 */
...
/* GPIO driver for the gpio command example */
...
/* ========================================================================== */
/*                    EXAMPLE 1: ECHO (string + int + flag)                   */
/* ========================================================================== */

/**
 * @brief Repeats a message N times, optionally in uppercase.
 *
...
/* ========================================================================== */
/*                    EXAMPLE 2: CALC (two ints + flag)                       */
/* ========================================================================== */

/**
 * @brief Simple calculator with optional verbose mode.
 *
...
/* ========================================================================== */
/*                    EXAMPLE 3: GPIO (complex, 6 arguments)                  */
/* ========================================================================== */

/**
 * @brief Structure to store current GPIO configuration
 */
typedef struct
{
  int pin;
...
/**
 * @brief Configure a GPIO pin with mode, pull, level, info and NVS save options.
 *
 * Usage: gpio --pin <0-48> --mode <in|out|od> [--pull <up|down|none>] [--level <0|1>] [-i] [-s]
 */
static int cmd_gpio(cli_context_t *ctx)
{
  ...
  int level = level_specified ? ctx->args[3].int_value : -1;
...
void app_main(void)
{
  ...
  cli_config_t cli_cfg = {
    ...
    .banner = "\n"
              ...
              "  gpio -p N -m MODE  - Configure GPIO (use --pull, -l, -i)\n"
              "\n"
  ...
  /* Register system commands (free, heap, version, restart, sleep) */
  ...
  /* Register WiFi commands (join, scan, disconnect) */
#if (CONFIG_ESP_WIFI_ENABLED || CONFIG_ESP_HOST_WIFI_ENABLED)
  register_wifi();
#endif

  /* Register NVS commands (get, set, erase) */
  register_nvs();

  /* Register CLI-API example commands using batch registration */
  const cli_command_t *cmds[] = {&echo_cmd, &calc_cmd, &gpio_cmd};
  for (size_t i = 0; i < sizeof(cmds) / sizeof(cmds[0]); i++)
  {
    ESP_ERROR_CHECK(cli_register_command(cmds[i]));
  }

  ESP_LOGI(TAG, "Advanced example commands registered: echo, calc, gpio + system/wifi/nvs");

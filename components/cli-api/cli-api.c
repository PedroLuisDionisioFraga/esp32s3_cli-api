...
static void cli_setup_prompt(const char *prompt_str) {
  ...
  if (!linenoiseIsDumbMode()) {
    snprintf(s_prompt, CLI_PROMPT_MAX_LEN - 1, LOG_COLOR_I "%s" LOG_RESET_COLOR,
             prompt_temp);
  } else {
...
esp_err_t cli_register_command(const cli_command_t *cmd) {
  ...
  for (int i = 0; i < cmd->arg_count; i++) {
    ...
    case CLI_ARG_TYPE_INT:
      if (arg->required) {
        reg_cmd->argtable[i] = arg_int1(arg->short_opt, arg->long_opt,
                                        arg->datatype, arg->description);
      } else {
        reg_cmd->argtable[i] = arg_int0(arg->short_opt, arg->long_opt,
                                        arg->datatype, arg->description);
      }
      break;
    ...
    case CLI_ARG_TYPE_STRING:
      if (arg->required) {
        reg_cmd->argtable[i] = arg_str1(arg->short_opt, arg->long_opt,
                                        arg->datatype, arg->description);
      } else {
        reg_cmd->argtable[i] = arg_str0(arg->short_opt, arg->long_opt,
                                        arg->datatype, arg->description);
      }
      break;
    ...
    case CLI_ARG_TYPE_FLAG:
      if (arg->required) {
        reg_cmd->argtable[i] =
            arg_lit1(arg->short_opt, arg->long_opt, arg->description);
      } else {
        reg_cmd->argtable[i] =
            arg_lit0(arg->short_opt, arg->long_opt, arg->description);
      }
      break;
  ...
  s_cmd_count++;
  ESP_LOGI(TAG, "Comando '%s' registrado com %d argumentos", cmd->name,
           cmd->arg_count);
...
esp_err_t cli_register_commands(const cli_command_t *commands, size_t count) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (ret != ESP_OK) {
      ESP_LOGE(TAG, "Falha ao registrar comando %zu: %s", i,
               esp_err_to_name(ret));
      return ret;
    }
  }
